#!/bin/bash
# douchebag — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/Stronger197/douchebag/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the douchebag hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/Stronger197/douchebag/main/hooks"

HOOK_FILES=("package.json" "douchebag-config.js" "douchebag-activate.js" "douchebag-mode-tracker.js" "douchebag-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if DOUCHEBAG_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.DOUCHEBAG_SETTINGS, 'utf8'));
      const hasDouchebagHook = (event) =>
      ...
      process.exit(
        hasDouchebagHook('SessionStart') &&
        hasDouchebagHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Douchebag hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/douchebag-activate.js" ]; then
  echo "Reinstalling douchebag hooks (--force)..."
else
  echo "Installing douchebag hooks..."
fi
...
chmod +x "$HOOKS_DIR/douchebag-statusline.sh"
...
DOUCHEBAG_SETTINGS="$SETTINGS" DOUCHEBAG_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.DOUCHEBAG_SETTINGS;
  const hooksDir = process.env.DOUCHEBAG_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/douchebag-statusline.sh';
...
echo "  - SessionStart hook: auto-loads douchebag rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/douchebag normal, /douchebag full, /douchebag ultra, /douchebag-commit, etc.)"
echo "  - Statusline badge: shows [DOUCHEBAG] or [DOUCHEBAG:ULTRA] etc."
